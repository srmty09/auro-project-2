#!/usr/bin/env python3
"""
Improved Odia-English Translation Training with Pre-trained T5 Model
This version uses T5 model fine-tuning for better translation performance.
"""
...
# Hugging Face transformers for pre-trained mT5 models
try:
    ...
    def __init__(self, config: ImprovedConfig):
        ...
        if TRANSFORMERS_AVAILABLE:
            try:
                ...
                # Use mT5 tokenizer which supports multiple languages including Odia
...
# ================================
# T5-BASED TRANSLATION MODEL WITH ROPE
# ================================
...
# ================================
# IMPROVED TRANSFORMER MODEL (LEGACY)
# ================================
...
class ImprovedOdiaEnglishDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.tokenizer.use_pretrained:
            # For T5, add task prefix to input
            ...
            # Validate token IDs to prevent CUDA device assert errors
...
class ImprovedTrainer:
    ...
    def __init__(self, model: ImprovedTransformerModel, tokenizer: ImprovedTokenizer, 
        ...
        if torch.cuda.is_available():
            ...
            # Setup multi-GPU if available and enabled
        ...
        # Setup optimizer with better parameters for T5 fine-tuning
        ...
        # Test dataset evaluation tracking
        self.test_evaluation_history = []
        
        # Test dataset will be loaded separately for comprehensive evaluation
        self.test_dataset = None
        
        # Test sentences for monitoring (kept for compatibility)
        self.test_sentences = [
            ("ମୁଁ ଭଲ ଅଛି", "I am fine"),
            ("ତୁମର ନାମ କଣ", "What is your name"),
            ("ଆଜି ଆବହାୱା ଭଲ", "The weather is good today"),
    ...
    def train_epoch(self, train_loader: DataLoader) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(pbar):
            try:
                ...
                # For T5, we use input_ids as source and labels as target
                input_ids = src_ids
                labels = tgt_ids
                
                # Debug: Print shapes and batch info for first batch
                if batch_idx == 0:
                ...
                if self.use_mixed_precision:
                    ...
                    # Final debug after processing (only for first batch)
                    if batch_idx == 0:
                    ...
                    # Backward pass with mixed precision (only if loss is finite)
                ...
                # Update parameters every accumulate_grad_batches steps
                if (batch_idx + 1) % self.accumulate_grad_batches == 0:
                    if self.use_mixed_precision:
                        # Only proceed if we have valid gradients (no NaN losses were encountered)
                        try:
                            ...
                            if torch.isnan(total_norm) or torch.isinf(total_norm):
                                ...
                                # Update learning rate scheduler only on successful steps
                            ...
                            # Try to recover by updating scaler and clearing gradients
                            try:
                                self.scaler.update()
                            except:
                                # If scaler update fails, recreate it
                    ...
                    # Zero gradients after each update attempt
                    self.optimizer.zero_grad()
                    
                    # Check for NaN in model parameters periodically and save history
    ...
    def test_translations(self) -> Dict[str, any]:
        ...
        # Use the comprehensive test dataset evaluation
    ...
    def evaluate_on_full_test_dataset(self):
        ...
        # Save to test evaluation history
        evaluation_result = {
            'epoch': self.current_epoch + 1,
    ...
    def print_training_progress(self):
        ...
        # Show improvement trends using BLEU scores
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader, test_loader: DataLoader):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            # Print progress summary every 3 epochs
            if (epoch + 1) % 3 == 0:
        ...
        # Show final translation examples
        if self.translation_history:
    ...
    def save_checkpoint(self, checkpoint_name: str):
        ...
        # Also save comprehensive training history logs
        if self.detailed_history['iterations']:
            ...
            with open(history_path, 'w', encoding='utf-8') as f:
                ...
                # Add epoch-wise test evaluation history
        ...
        # Save epoch-wise translation summary
        if self.translation_history:
...
# ================================
# GPU RESET AND RECOVERY FUNCTIONS
# ================================
...
def main():
    ...
    try:
        ...
        # Create mT5-based model with safe error recovery
